    ...
    <section className="relative w-full h-full flex flex-col justify-start md:justify-start lg:justify-center overflow-visible md:overflow-hidden pointer-events-none pt-12 sm:pt-16 md:pt-32 lg:pt-0 pb-6 md:pb-0">
      <div className="container mx-auto px-4 md:px-6 relative z-10 pointer-events-auto">
        <motion.div
          ...
          viewport={{ once: false }}
          className="mb-3 sm:mb-4 md:mb-6 lg:mb-20"
        >
          ...
          <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-6xl font-display text-white leading-tight max-w-4xl">
            Мы создаем <span className="inline-block pt-[1px] pb-[1px] leading-[1.1] text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-blue-600">будущее</span> сегодня
          </h2>
        </motion.div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 md:gap-2 lg:gap-6">
          {PHILOSOPHY_STATS.map((stat, index) => (
            <motion.div 
              ...
              className="tech-border bg-[#0A0A0F]/90 backdrop-blur-xl p-2 md:p-2 lg:p-6 group hover:bg-[#0F1025] transition-colors shadow-lg border-cyan-500/20"
            >
              ...
              <p className="text-gray-300 font-mono uppercase tracking-[0.15em] text-[10px] md:text-[11px] lg:text-sm border-t border-white/10 pt-1 md:pt-2 lg:pt-4 mt-1 md:mt-2 lg:mt-4 group-hover:border-cyan-500/50 transition-colors">
                  {stat.label}
              </p>
            </motion.div>
          ))}
        ...
        <motion.div 
          className="mt-3 md:mt-8 lg:mt-24 p-2 md:p-4 lg:p-8 border-l-4 border-cyan-500 bg-[#0A0A0F]/80 backdrop-blur-md shadow-2xl"
          initial={{ opacity: 0, x: 100 }}
          whileInView={{ opacity: 1, x: 0 }}
          viewport={{ once: false }}
          transition={{ delay: 0.4, duration: 1 }}
        >
          <p className="text-xs sm:text-sm md:text-sm lg:text-xl xl:text-2xl text-white leading-relaxed max-w-4xl">
